...
{
	//configure Board ADS chip select pin
	Drv_GPIO_setPinState(BOARD_ADS_PORT, BOARD_ADS_PIN, TRUE); //set HIGH

	//configure Slave ADS chip select pin
	...
	//configure ADS POWER DOWN pin
    Drv_GPIO_setPinState(ADS_PWDN_PORT, ADS_PWDN_PIN, TRUE); //set HIGH no power down

	//TODO: configure and begin SPI
	Drv_SSP_init();
	Drv_SSP_setup();
...
{
	//TODO: Recommended power up sequence requiers >Tpor (aprox. 32mS)
	...
	Drv_ADS1299_WREG(CONFIG1, 0xB0 | SAMPLE_RATE_1kHZ, BOARD_ADS); //tell on-board ADS to output its clk, set the data rate to 250SPS
	...
	if(!slavePresent)
	{
		Drv_ADS1299_WREG(CONFIG1, 0x90 | SAMPLE_RATE_250HZ, BOARD_ADS); //Turn off clk output if no slave present
		numChannels = 8; //expect up to 8 ADS channels
	}
	else
	{
		Drv_ADS1299_WREG(CONFIG1, 0x90 | SAMPLE_RATE_250HZ, SLAVE_ADS);//Slow down sample rate for 16 channels
    ...
    defaultChannelSettings[POWER_DOWN_] = NO;        // on = NO, off = YES
    defaultChannelSettings[GAIN_SET] = ADS_GAIN12;     // Gain setting
...
uint8_t Drv_ADS1299_xfer(uint8_t _data)
{
	 ...
	 inByte = spi_rw(_data); // Faster transmission
...
void Drv_ADS1299_writeOneChannelSettings(uint8_t N)
{
	...
	if(channelSettings[N][POWER_DOWN_] == YES)
	{
		setting |= 0x80;
	}
...
{
	uint8_t gpio = 0;
	gpio = Drv_ADS1299_RREG(GPIO_, BOARD_ADS);
...
{
	//TODO: Revisar la la velocidad de transmision de datos
	//Drv_ADS1299_WREG(CONFIG1, 0XB6, BOARD_ADS); //tell on-board ADS to output the clk, set the data rate to 250SPS
	...
	if(!slavePresent)
	{
		//Drv_ADS1299_WREG(CONFIG1, 0x96, BOARD_ADS); //turn off clk output if no daisy present
